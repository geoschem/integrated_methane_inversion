...
slurm_info = subprocess.run(['/usr/sbin/slurmd', '-C'], stdout=subprocess.PIPE).stdout.decode('utf-8').split()
...
with open('/home/ubuntu/integrated_methane_inversion/envs/aws/slurm/new_slurm.conf', 'a+') as f:
    f.write('\n')
    f.write(first_line)
    f.write(second_line)
    f.write(third_line)
