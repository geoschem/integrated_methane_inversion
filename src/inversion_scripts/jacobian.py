...
def average_tropomi_observations(TROPOMI, gc_lat_lon, sat_ind):
    ...
    for k in range(n_obs):
        ...
        corners_lon_index = []
        corners_lat_index = []
        gc_lats = [item["lat"] for item in gc_lat_lon]
        gc_lons = [item["lon"] for item in gc_lat_lon]
        for l in range(4):
        ...
        if not total_overlap_area == 0:
            ...
            gridcell_dict = next(item for item in gc_lat_lon if item["lat"] == gc_coords[max_index][1] and item["lon"] == gc_coords[max_index][0] )
            ...
            gridcell_dict["time"].append(pd.to_datetime(str(TROPOMI["utctime"][iSat])))
    ...
    gridcell_dicts = [item for item in gc_lat_lon if item["observation_count"] > 0]
    ...
    for gridcell_dict in gridcell_dicts:
        ...
        gridcell_dict["p_sat"] = np.mean(gridcell_dict["p_sat"])
        gridcell_dict["dry_air_subcolumns"] = np.mean(gridcell_dict["dry_air_subcolumns"])
        gridcell_dict["apriori"] = np.mean(gridcell_dict["apriori"])
        gridcell_dict["avkern"] = np.mean(gridcell_dict["avkern"])
        gridcell_dict["time"] = np.mean(gridcell_dict["time"]).round("60min").strftime("%Y%m%d_%H")
...
def get_gc_lat_lon(gc_cache, start_date):
    ...
    filename = f"{gc_cache}/{file_species}"
    gc_data = xr.open_dataset(filename)
    lons = gc_data["lon"].values
    lats = gc_data["lat"].values

    # create array of dictionaries to represent gridcells
    for i in range(len(lons)):
        key = f"{lats[i]}_{lons[i]}"
        gridcell_index_dict[key] = i #TODO decide what to do with this
        dat.append({
            "lat": lats[i],
            "lon": lons[i],
            "methane": [],
...
# checks if observation is within a GC gridcell
def obs_gc_grid_comparison(iSat, jSat, TROPOMI, all_date_gc):
    obs_gridcell_info = {}
    ...
    # If the tolerance in nearest_loc() is not satisfied, skip the observation
    if np.nan in corners_lon_index + corners_lat_index:
        obs_gridcell_info["valid"] = False
        return obs_gridcell_info

    # Get lat/lon indexes and coordinates of GEOS-Chem grid cells closest to the TROPOMI corners
    obs_gridcell_info["ij_GC"] = [(x, y) for x in set(corners_lon_index) for y in set(corners_lat_index)]
    obs_gridcell_info["gc_coords"] = [(GEOSCHEM["lon"][i], GEOSCHEM["lat"][j]) for i, j in obs_gridcell_info["ij_GC"]]
    
    # Compute the overlapping area between the TROPOMI pixel and GEOS-Chem grid cells it touches
    overlap_area = np.zeros(len(obs_gridcell_info["gc_coords"]))
    ...
    for gridcellIndex in range(len(obs_gridcell_info["gc_coords"])):
        # Define polygon representing the GEOS-Chem grid cell
        coords = obs_gridcell_info["gc_coords"][gridcellIndex]
        geoschem_corners_lon = [
            coords[0] - dlon / 2,
            coords[0] + dlon / 2,
            coords[0] + dlon / 2,
    ...
    # If there is no overlap between GEOS-Chem and TROPOMI, skip to next observation:
    obs_gridcell_info["overlap_areas"] = overlap_area
    obs_gridcell_info["total_overlap_area"] = sum(overlap_area)
    obs_gridcell_info["valid"] = not obs_gridcell_info["total_overlap_area"] == 0
    return obs_gridcell_info
...
def apply_average_tropomi_operator(
    ...
    for i, gridcell_dict in enumerate(obs_mapped_to_gc):
        ...
        p_gc = GEOSCHEM["PEDGE"][gridcell_dict["lon"], gridcell_dict["lat"], :]
        # Get GEOS-Chem methane for the cell
        gc_CH4 = GEOSCHEM["CH4"][gridcell_dict["lon"], gridcell_dict["lat"], :]
        ...
        if build_jacobian:
            ...
            sensi_lonlat = GEOSCHEM["Sensitivities"][gridcell_dict["lon"], gridcell_dict["lat"], :, :]
...
def apply_tropomi_operator(
    ...
    for k in range(n_obs):
        ...
        obs_gridcell_info = obs_gc_grid_comparison(iSat, jSat, TROPOMI, all_date_gc)
        
        # check if observation is in region of interest
        if obs_gridcell_info["valid"]:
            continue
        ...
        for gridcellIndex in range(len(obs_gridcell_info["gc_coords"])):
            ...
            iGC, jGC = obs_gridcell_info["ij_GC"][gridcellIndex]
            ...
            # Weight by overlapping area (to be divided out later) and add to sum
            area_weighted_virtual_tropomi += (
                obs_gridcell_info["overlap_areas"][gridcellIndex] * virtual_tropomi_gridcellIndex
            )  # ppb m2
            ...
            if build_jacobian:
                ...
                # Weight by overlapping area (to be divided out later) and add to sum
                area_weighted_virtual_tropomi_sensitivity += (
                    obs_gridcell_info["overlap_areas"][gridcellIndex] * tropomi_sensitivity_gridcellIndex
                )  # m2

        # Compute virtual TROPOMI observation as weighted mean by overlapping area
        # i.e., need to divide out area [m2] from the previous step
        virtual_tropomi = area_weighted_virtual_tropomi / obs_gridcell_info["total_overlap_area"]
        ...
        if build_jacobian:
            # Compute TROPOMI sensitivity as weighted mean by overlapping area
            # i.e., need to divide out area [m2] from the previous step
            jacobian_K[k, :] = area_weighted_virtual_tropomi_sensitivity / obs_gridcell_info["total_overlap_area"]
