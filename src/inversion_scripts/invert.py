...
def do_inversion(
    ...
    for fi in files:
        ...
        if train_test_split:
            ...
            test_tropomi_obs = np.concatenate([test_tropomi_obs, obs_GC[test_ind, 0]])
            test_prior_obs = np.concatenate([test_prior_obs, obs_GC[test_ind, 1]])
        ...
        if train_test_split:
            if 'K_test' not in locals():
                ...
                K_train = np.append(K_train, K, axis=0)
