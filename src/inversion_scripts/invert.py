...
def do_inversion(
    ...
    test_diff_prior = np.array([]) # used only for train/test split
    # Initialize
    ...
    for fi in files:
        ...
        if train_test_split:
            ...
            # Assuming 'data' is your numpy array with shape (x, 5)
            num_obs = len(ind)  # Number of observations
            rng = np.random.RandomState(42)
            indices = rng.permutation(ind)  # Randomly shuffle the indices

            # Split the shuffled indices for an 80-20 split
            train_size = int(0.8 * num_obs)
            ind = np.sort(indices[:train_size])
            test_ind = np.sort(indices[train_size:])
            test_diff_prior = np.concatenate([test_diff_prior, obs_GC[test_ind, 0] - obs_GC[test_ind, 1]])
        ...
        if train_test_split:
            if 'K_test' not in locals():
                K_test = 1e9 * K_dat[test_ind, :]
            else:
                K_test = np.append(K_test, 1e9 * K_dat[test_ind, :], axis=0)    
    ...
    if train_test_split:
        # return rmse 
        return np.sqrt(np.mean((K_test@xhat) - test_diff_prior) ** 2)
    else:
...
if __name__ == "__main__":
    ...
    if optimize_gamma:
        ...
        for i in range(len(rmses)):
            print(f"gamma: {gammas[i]}, rmse: {rmses[i]}")
