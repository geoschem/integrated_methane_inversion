...
def read_geoschem(date, gc_cache, n_elements, config, build_jacobian=False):
    ...
    if UseGCHP:
        ...
        species = gc_data["SpeciesConcVV_{config['Species']}"].values[0, ...]
    ...
    if build_jacobian:
        ...
        if UseGCHP:
            dat["emis_base_species"] = np.einsum("klfyx->fyxlk", ds_emis_base["Species"].values)
        else:
            dat["emis_base_species"] = np.einsum("klji->ijlk", ds_emis_base["Species"].values)
        ...
        if UseGCHP:
            dat["oh_base_species"] = np.einsum("klfyx->fyxlk", ds_oh_base["Species"].values)
        else:
            dat["oh_base_species"] = np.einsum("klji->ijlk", ds_oh_base["Species"].values)
...
def concat_tracers(run_id, gc_date, config, sv_elems, n_elements, baserun=False):
    ...
    with xr.open_dataset(
        ...
        ds_concat = xr.concat(
            (dsmf[v] for v in keepvars), dim="element"
