...
def make_state_vector_file(
    ...
    if UseGCHP:
        gridds = xr.open_dataset('./CS_grids/grids.c{}.nc'.format(CS_RES))
        ...
        transform = sparselt.esmf.load_weights(
            './CS_grids/regrid_weights_{}_to_c{}_conserve.nc'.format(nativeres, CS_RES),   # Generated by ESMF_RegridWeightGen
    ...
    if not UseGCHP:
        ...
        indomain_mask = (
            (lons >= lon_min_inv_domain) & (lons <= lon_max_inv_domain) &
            (lats >= lat_min_inv_domain) & (lats <= lat_max_inv_domain)
        )
